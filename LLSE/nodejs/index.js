...
const { loadBDX } = require(path.resolve(__dirname, "./bdxDecode.js"));
...
// ----- AMCBuilder i18n -----
if (!File.exists("plugins/AMCBuilder/language/zh_CN.json")) File.writeTo("plugins/AMCBuilder/language/zh_CN.json", JSON.stringify({
...
function AMCBCommandHandle(sender, args) {
   ...
   if (!sender.isOP()) {
	...
	switch (args[0]) {
		...
		case "test": {
			var block = mc.getBlock(sender.blockPos);
			sender.tell([block.pos.x, block.pos.y, block.pos.z, block.type, block.id, block.tileData].join("、"));
			sender.tell(block.getNbt().toString());
			break;
		}
		default: {
			sender.tell(helpStr);
...
class AMCBPlayer__ {
	...
	updatePos(pos, index) {
		...
		genEdgeParticles(this.data["pos"][0], this.data["pos"][1], dimid, this.ReginSize);
		return true;
	}
	wsConnect() {
...
var CommandList =[
	...
	["amcb pos1", '设置第一个坐标'],
	["amcb pos2", '设置第二个坐标'],
	["amcb import <filename> [-air] [--txt,--mcs] [-loca]", '导入本地建筑文件'],
...
async function cmdFillHandle(sender, args) {
	var AMCBPlayer = sender.getExtraData("AMCBPlayer__");
	const fileName = args.length > 1 ? args[1] : generateMixed(5);
	AMCBPlayer.tell('§a[AMCBuilder] 开始填充');
	const startTime = new Date().getTime();
	if (File.readFrom("./plugins/AMCBuilder/export/"+fileName+".txt")) {
		return AMCBPlayer.tell('filename_repeat', 0, [header, fileName]);
	}
	...
	for (let x = minPos[0]; x<= maxPos[0]; ++x) {
		for (let z = minPos[2]; z<= maxPos[2]; ++z) {
			for (let y = minPos[1]; y<= maxPos[1]; ++y) {
				mc.setBlock(x, y, z, AMCBPlayer.data.dimid, args[1], args[2]||0);
			}
		}
	}
	AMCBPlayer.sendToast('§b[AMCBuilder]', '填充结束 耗时:'+((new Date().getTime() - startTime)/1000).toFixed(2)+'s');
}
...
function genEdgeParticles(pos1, pos2, dimid, size) {
	...
	mc.spawnParticle(pos1[0]+0.5, pos1[1], pos1[2], dimid, Config.particles.name);
	mc.spawnParticle(pos1[0], pos1[1]+0.5, pos1[2], dimid, Config.particles.name);
	mc.spawnParticle(pos1[0], pos1[1], pos1[2]+0.5, dimid, Config.particles.name);
	mc.spawnParticle(pos2[0]+1, pos2[1]+1, pos2[2]+1, dimid, Config.particles.name);
	mc.spawnParticle(pos2[0]+0.5, pos2[1]+1, pos2[2]+1, dimid, Config.particles.name);
	mc.spawnParticle(pos2[0]+1, pos2[1]+0.5, pos2[2]+1, dimid, Config.particles.name);
	mc.spawnParticle(pos2[0]+1, pos2[1]+1, pos2[2]+0.5, dimid, Config.particles.name);
	if (size < 2) {
		return;
	}
}
