...
import { Player } from "@LLSELib/object/Player.js"; // 用于code补全
...
export function main() {
    ...
    if (!File.exists("plugins/AMCBuilder/language/zh_CN.json"))
        File.writeTo(
            "plugins/AMCBuilder/language/zh_CN.json",
            JSON.stringify(
                {
                    ...
                    failblocks_tips:
                        "§b{}任然有 {} 方块未能成功导入，请在控制台查看。",
                    connect_amcb_server_first:
                        "§c{}请先连接至 AMCBuilder 服务器",
                    pls_select_build_target: "§c{}请先选择建筑",
                    import_build_start:
                        "§b{}开始导入 导入过程可能较长 期间服务器可能无法正常处理事务",
                    import_build_ing:
                        "§b{}请等待构建完毕，断开链接将强行停止构建",
    ...
    if (!File.exists("plugins/AMCBuilder/language/en.json"))
        File.writeTo(
            "plugins/AMCBuilder/language/en.json",
            JSON.stringify(
                {
                    set_pos_succ: "{} Setting pos {} succeeded {} ({})",
                    filename_repeat:
                        "§c{}The file name {} is duplicate, please pass in the file name again.",
                    less_1_points: "§c{}Please set pos1 first.",
                    less_2_points: "§c{}Please set pos1 and pos2 first.",
                    need_op_perm: "§c{}You need OP permission.",
                    failblocks_tips:
                        "§b{}There are still {} blocks that could not be imported successfully, please check on the logger.",
                    connect_amcb_server_first:
                        "§c{}Please connect to the AMCBuilder server first.",
                    pls_select_build_target:
                        "§c{}Please select a building first.",
                    import_build_start:
                        "§b{}Starting the import process may take a long time. The server may not be able to process transactions normally.",
                    import_build_ing:
                        "§b{}Please wait until the construction is completed. Disconnecting the link will forcibly stop the construction.",
                    pos_level_invaild:
                        "§c{}The current world dimension is invalid!",
                    pos_level_update: "{}The dimension has been updated to {}",
                    reject_console:
                        "§c{}Please use the player to execute the command",
                },
                null,
                2
            )
    ...
    var CommandList = [
        ...
        ["amcb pos2", "设置第二个坐标"],
        [
            "amcb import <filename> [-air] [--txt,--mcs] [-loca]",
            "导入本地建筑文件",
        ],
        ["amcb export <filename> [-air] [--txt,--mcs]", "导出选区的建筑物"],
        ["amcb fill <blockId> [metaId]", "填充指定区域"],
        ["amcb copy", "复制指定区域到剪贴板"],
        ["amcb paste", "粘贴剪贴板到pos1"],
        ["amcb cut", "剪切到剪贴板"],
        ["amcb pos2", "设置第二个坐标"],
    ...
    mc.listen("onServerStarted", () => {
        ...
        cmd.setEnum("FillAction", ["fill", "set"]);
        cmd.setEnum("CopyAction", ["copy"]);
        cmd.setEnum("PasteAction", ["paste"]);
        ...
        cmd.mandatory("action", ParamType.Enum, "FillAction", 1);
        cmd.mandatory("action", ParamType.Enum, "CopyAction", 1);
        cmd.mandatory("action", ParamType.Enum, "PasteAction", 1);
        ...
        cmd.optional("dimId", ParamType.Int);
        cmd.optional("metaId", ParamType.Int);
        ...
        cmd.overload(["PasteAction"]);
        cmd.overload(["CutAction"]);
    ...
    function AMCBCommandHandle(_cmd, _ori, out, res) {
        ...
        if (!sender.getExtraData("AMCBPlayer__")) {
            sender.setExtraData(
                "AMCBPlayer__",
                new AMCBPlayer__(sender.realName)
            );
        }
        ...
        switch (res.action) {
            ...
            case "pos1": {
                ...
                AMCBPlayer.tell("set_pos_succ", 0, [
                    header,
                    "1",
                    AMCBPlayer.data.pos_[0].join(", "),
                    AMCBPlayer.ReginSize,
                ]);
                break;
            }
            case "2":
            ...
            case "fill":
            case "set": {
                cmdFillHandle(sender, [res.blockId.type, res.metaId || 0]);
                break;
            }
            case "copy": {
                cmdCopyHandle(sender);
                break;
            }
            case "cut": {
                cmdCopyHandle(sender);
                cmdFillHandle(sender, ["minecraft:air", 0]);
                break;
            }
            case "paste":
            case "pastes": {
                cmdPasteHandle(sender);
                break;
            }
            case "refail": {
    ...
    class AMCBPlayer__ {
            ...
            if (!player) {
                logger.info(
                    "offline toast to " +
                        this.name +
                        ":\n  title:" +
                        i18n.trl(player.langCode, title, args) +
                        "  message:" +
                        msg
                );
                return false;
        ...
        reFailBlocks() {
            ...
            for (const block of failBlocks) {
                if (
                    mc.getBlock(block[0], block[1], block[2], this.data.dimid)
                        .type != block[3] &&
                    !mc.setBlock(
                        block[0],
                        block[1],
            ...
            if (this.failBlocks.length)
                this.tell("failblocks_tips", 0, [
                    header,
                    this.failBlocks.length,
                ]);
        }
        startBuild() {
        ...
        updatePos(pos, index) {
            ...
            let dimid = isPNX
                ? pos.dim || this.player().blockPos.dim
                : pos.dimid;
            if (this.data.dimid != dimid) {
                if (pos.dimid === -1) {
                    this.tell("pos_level_invaild", 0, [header]);
                } else {
                    this.tell("pos_level_update", 0, [
                        header,
                        isPNX ? dimid : worldDimid[dimid],
                    ]);
            ...
            var xMin = min(
                this.data["pos_"][index][0],
                this.data["pos_"][anti][0]
            );
            var xMax = max(
                this.data["pos_"][index][0],
                this.data["pos_"][anti][0]
            ...
            var yMin = min(
                this.data["pos_"][index][1],
                this.data["pos_"][anti][1]
            );
            var yMax = max(
                this.data["pos_"][index][1],
                this.data["pos_"][anti][1]
            ...
            var zMin = min(
                this.data["pos_"][index][2],
                this.data["pos_"][anti][2]
            );
            var zMax = max(
                this.data["pos_"][index][2],
                this.data["pos_"][anti][2]
        ...
        updataSetup(config) {
            if (
                !File.exists(
                    "plugins/AMCBuilder/playerSetup/" + this.name + ".json"
                )
            ) {
                File.writeTo(
                    "plugins/AMCBuilder/playerSetup/" + this.name + ".json",
                        ...
                        case "nextCmd": {
                            ...
                            for (const block of blockList) {
                                ...
                                if (
                                    ...
                                    logger.info(
                                        "Failed place: " + JSON.stringify(block)
                                    );
                                    this.failBlocks.push([
                                        block[0],
                                        block[1],
                            ...
                            if (cmdList.length < 1000) {
                                ...
                                if (this.failBlocks.length) {
                                    this.tell(
                                        "§b[AMCBuilder] 有 " +
                                            this.failBlocks.length +
                                            " 方块未能成功导入，使用 /amcb refail 重新导入错误方块"
                                    );
                                }
                                this.tell(
                                    "§a[AMCBuilder] 导入结束\n  §a用时: " +
                                ...
                                this.tell(
                                    "§b[AMCBuilder] §e" +
                                        ...
                                        " §b已完成§e" +
                                        (
                                            this.nowBlockId / this.totalBlock
                                        ).toFixed(2),
                                    5
                                );
                            }
        ...
        wsDisconnect() {
            if (this.ws.status)
                return this.tell(header + "请勿重复断开与服务器的链接");
            this.ws.close();
        ...
        noiseGen() {
            ...
            for (let x = xMin; x < xMax; x++) {
                for (let z = zMin; z < zMax; z++) {
                    const y = noiseGen.no(
                        Math.abs((x - xMin) / (xMax - xMin)),
                        Math.abs((z - zMin) / (zMax - zMin))
                    );
                    mc.setBlock(
                        x,
                        this.data["pos"][0][1] + y,
    ...
    var noiseGen = {
        ...
        ya0: function (x) {
            return (
                this.K1_0 ||
                this.getRandom(-5, 5) * x + this.B1_0 ||
                this.getRandom(-5, 5)
            );
        },
        ya1: function (x) {
            return (
                this.K1_1 ||
                this.getRandom(-5, 5) * x + this.B1_1 ||
                this.getRandom(-5, 5)
            );
        },
        na: function (x) {
        ...
        yb0: function (x) {
            return (
                this.K2_0 ||
                this.getRandom(-5, 5) * x + this.B2_0 ||
                this.getRandom(-5, 5)
            );
        },
        yb1: function (x) {
            return (
                this.K2_1 ||
                this.getRandom(-5, 5) * x + this.B2_1 ||
                this.getRandom(-5, 5)
            );
        },
        nb: function (x) {
        ...
        for (let x = 0; x < pos[0]; x++) {
            for (let y = 0; y < pos[1]; y++) {
                for (let z = 0; z < pos[2]; z++) {
                    const block =
                        paletteList[index] > -1
                            ? blockPalette[paletteList[index]]
                            : {
                                  name: "minecraft:air",
                                  states: {},
                                  version: 17959425,
    ...
    async function cmdExportHandle(sender, args) {
        var AMCBPlayer = sender.getExtraData("AMCBPlayer__");
        const fileName = args.length > 1 ? args[0] : generateMixed(5);
        ...
        if (isSolid)
            AMCBPlayer.tell(
                "§7[AMCBuilder] 本次导出将过滤空气方块 添加 -air参数可保留"
            );
        await new Promise((resolve, reject) => {
            setTimeout(function () {
                AMCBPlayer.tell(
                    "§b[AMCBuilder] 开始导出 总计" +
                        AMCBPlayer.ReginSize +
                        "个方块 预计需要" +
                        ((AMCBPlayer.ReginSize / 3e4) >> 0) +
                        "s 期间服务器可能无法正常处理事务"
                );
            }, 200);
            setTimeout(function () {
        ...
        if (args.indexOf("--txt") > -1) {
            ...
            if (cmdList.length === 0) {
                return AMCBPlayer.tell(
                    "§c[AMCBuilder] 本次导出数据为空，请重新选择范围。"
                );
            }
            AMCBPlayer.tell(
                "§a[AMCBuilder] 文件导出在 ./plugins/AMCBuilder/export/" +
                    fileName +
                    ".txt\n  §a用时: " +
                    (finishTime / 1000).toFixed(2) +
                    " s\n  §a速度: " +
                    (((cmdList.length / finishTime) * 1000) >> 0) +
                    " B/s"
            );
            File.writeTo(
                "./plugins/AMCBuilder/export/" + fileName + ".txt",
                cmdList.join("\n")
            );
        } else {
            try {
                    ...
                    for (let y = minPos[1]; y <= maxPos[1]; ++y) {
                        ...
                        for (let z = minPos[2]; z <= maxPos[2]; ++z) {
                            ...
                            if (index === -1) {
                                ...
                                snbt.structure.palette.default.block_palette.push(
                                    JSON.parse(blockSNBT)
                                );
                                index =
                                    snbt.structure.palette.default.block_palette
                                        .length - 1;
                            } else {
                                index++;
                            }
                ...
                const finishTime = new Date().getTime() - startTime;
                const comp = NBT.parseSNBT(
                    JSON.stringify(snbt)
                        .replace(/_bit":0/g, '_bit":0b')
                        .replace(/_bit":1/g, '_bit":1b')
                );
                const path =
                    "./plugins/AMCBuilder/export/" + fileName + ".mcstructure";
    ...
    function cmdImportHandle(sender, args) {
        ...
        try {
            if (typeof AMCBPlayer.data.pos[0] != "object") throw "pos1";
            startPos = AMCBPlayer.data.pos[0];
        } catch (e) {
        ...
        if (args.length < 2 || !args[0]) {
            ...
            fm.addDropdown(
                "选择建筑文件",
                ["取消", ...File.getFilesList("./plugins/AMCBuilder/export/")],
                0
            );
        ...
        if (args.indexOf("--txt") > -1) {
            ...
            if (!fileContext) {
                return AMCBPlayer.tell(
                    "§c[AMCBuilder] 文件读取错误./plugins/AMCBuilder/export/" +
                        args[0] +
                        ".txt"
                );
            }
            ...
            list1.forEach((v, i) => {
                ...
                if (
                    mc
                        .getBlock(
                            Number(row[0]) + startPos[0],
                            Number(row[1]) + startPos[1],
                            Number(row[2]) + startPos[2],
                            AMCBPlayer.data.dimid
                        )
                        .type.indexOf(row[3]) == -1 &&
                    !mc.setBlock(
                        Number(row[0]) + startPos[0],
                        Number(row[1]) + startPos[1],
                        Number(row[2]) + startPos[2],
            ...
            if (!loadBDX) {
                AMCBPlayer.tell(
                    "§c[AMCBuilder] 缺少loadBDX前置，无法解析bdx文件"
                );
            }
            const startTime = new Date().getTime();
            var cmdCount = 0;
                            ...
                            switch (data.type) {
                                ...
                                case "cb_data": {
                                    ...
                                    if (block && block.hasBlockEntity()) {
                                        ...
                                        block.getBlockEntity().setNbt(
                                            new NbtCompound({
                                                Command: new NbtString(
                                                    data.Command || ""
                                                ),
                                                CustomName: new NbtString(
                                                    data.CustomName || ""
                                                ),
                                                ExecuteOnFirstTick: new NbtByte(
                                                    data.ExecuteOnFirstTick
                                                        ? 1
                                                        : 0
                                                ),
                                                LPCommandMode: new NbtInt(
                                                    data.LPCommandMode
                                                ),
                                                LPCondionalMode: new NbtByte(
                                                    data.LPCondionalMode ? 1 : 0
                                                ),
                                                LPRedstoneMode: new NbtByte(
                                                    data.LPRedstoneMode ? 1 : 0
                                                ),
                                                LastExecution: new NbtLong(0),
                                                LastOutput: new NbtString(
                                                    data.LastOutput || ""
                                                ),
                                                LastOutputParams: new NbtList(
                                                    data.LastOutputParams || []
                                                ),
                                                SuccessCount: new NbtInt(0),
                                                TickDelay: new NbtInt(
                                                    data.TickDelay
                                                ),
                                                TrackOutput: new NbtByte(
                                                    data.TrackOutput ? 1 : 0
                                                ),
            ...
            readMCStructure(
                ...
                function (status, data) {
                    switch (status) {
                        case 0: {
                            mc.setBlock(
                                Number(data[0]) + startPos[0],
                                Number(data[1]) + startPos[1],
                                Number(data[2]) + startPos[2],
                                AMCBPlayer.data.dimid,
                                NBT.parseSNBT(
        ...
        if (AMCBPlayer.failBlocks.length)
            AMCBPlayer.tell(
                "§b[AMCBuilder] 有 " +
                    AMCBPlayer.failBlocks.length +
                    " 方块未能成功导入，使用 /amcb refail 重新导入错误方块"
            );
        AMCBPlayer.tell(
            "§a[AMCBuilder] 导入结束\n  §a用时: " +
                (finishTime / 1000).toFixed(2) +
                " s\n  §a速度: " +
                (((cmdCount / finishTime) * 1000) >> 0) +
                " B/s"
        );
    }
    async function cmdCopyHandle(sender) {
        var AMCBPlayer = sender.getExtraData("AMCBPlayer__");
        try {
            if (typeof AMCBPlayer.data.pos[0] != "object") throw "pos1";
            if (typeof AMCBPlayer.data.pos[1] != "object") throw "pos2";
        ...
        const startTime = new Date().getTime();
        AMCBPlayer.copy_palette = mc.getStructure(
            mc.newIntPos(...[...AMCBPlayer.data.pos[0], AMCBPlayer.data.dimid]),
            mc.newIntPos(...[...AMCBPlayer.data.pos[1], AMCBPlayer.data.dimid])
        );
        const finishTime = new Date().getTime() - startTime;
        AMCBPlayer.sendToast(
            "§b[AMCBuilder]",
            "复制成功 用时:" + (finishTime / 1000).toFixed(2) + "s"
        );
    }
    async function cmdPasteHandle(sender) {
        var AMCBPlayer = sender.getExtraData("AMCBPlayer__");
        try {
            if (typeof AMCBPlayer.data.pos[0] != "object") throw "pos1";
        } catch (e) {
        ...
        if (!AMCBPlayer.copy_palette) {
            return AMCBPlayer.tell(
                "§c[AMCBuilder] 请先用 §f/amcb copy §c复制选区"
            );
        }
        const startTime = new Date().getTime();
        mc.setStructure(
            AMCBPlayer.copy_palette,
            mc.newIntPos(...[...AMCBPlayer.data.pos_[0], AMCBPlayer.data.dimid])
        );
        const finishTime = new Date().getTime() - startTime;
        AMCBPlayer.sendToast(
            "§b[AMCBuilder]",
            "粘贴成功 用时:" + (finishTime / 1000).toFixed(2) + "s"
        );
    }
    ...
    function cmdFillHandle(sender, args) {
        ...
        AMCBPlayer.sendToast(
            "§b[AMCBuilder]",
            "填充结束 用时:" + (finishTime / 1000).toFixed(2) + "s"
        );
    }
    ...
    mc.listen("onUseItemOn", function (player, item, block, side, pos) {
        ...
        if (!player.getExtraData("AMCBPlayer__")) {
            player.setExtraData(
                "AMCBPlayer__",
                new AMCBPlayer__(player.realName)
            );
        }
